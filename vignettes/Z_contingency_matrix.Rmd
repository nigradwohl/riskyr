...
calc_tab_rec <- function(table) {
  ...
  ## (A) Frequency case: 
    ...
    ## (a) Rows: 
      ...
      if (n_row %in% col_ix) {  # test, whether the maximal row is included (row sum).
        
        ## If TRUE, subtract the sum of the other values from their sum:
        table[row_ix,][is.na(table[row_ix,])] <- table[row_ix, n_col] - cell_sums_row
        
      } else {
        ## If FALSE (i.e., the sum is not included) simply put sum into respective cell:
        table[row_ix, n_col] <- cell_sums_row
      }
