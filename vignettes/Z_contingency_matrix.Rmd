...
calc_tab_rec <- function(table) {
  ...
  ## (A) Frequency case: 
    ...
    log_na <- is.na(ex_tab)  # get matrix of NAs
    
    col_ix <- which(colSums(log_na) == 1)
    ...
    ## (a) Rows: -----
      ...
      cell_sums_row <- rowSums(table[rowSums(log_na) == 1, -n_col], na.rm = TRUE)
      ...
      ## Test, which operation to perform: 
        ...
        if (any(!no_rsum_col)) {  
          ...
          table[cell_ix_row, no_cell_col[, 2]] <- insertion
    ...
    ## (b) Columns: -----
      ...
      ## Sum up values in the indexed rows except their sum in the last column: 
      cell_sums_row <- rowSums(table[row_ix, -n_col], na.rm = TRUE)
      
      ## Test, which operation to perform: 
        ## Test for empty sum columns in the calculable rows:
        no_rsum_col <- is.na(table[row_ix, 3])  # which of the rows is NA in the last column? 
        
        if (any(no_rsum_col)) {
          ## Check, which last columns in the identified rows are NA:
          cell_ix_row <- which(no_rsum_col)  
          ## get index for the row(s) in which the last column is empty. 
          table[row_ix, 3][is.na(table[row_ix, 3])] <- cell_sums_row[cell_ix_row]
          ## Enter appropriate values. 
        } 
        ...
        if (any(!no_rsum_col)) {  
          ## Check, which columns in the identified rows are NA:
          cell_ix_row <- row_ix[!no_rsum_col]
          row_NA <- table[cell_ix_row, ]  # get the rows with NAs not in last position. 
          no_cell_col <- which(is.na(row_NA), arr.ind = TRUE)  # index for the cells without entries. 
          
          table[cell_ix_row, no_cell_col[, 2]] <- table[row_ix, n_col] - cell_sums_row[!no_rsum_col]
