...
## Function for recursive generation: 
calc_tab <- function(table) {
  ...
  ## (A) Frequency case: -----
    ...
    ## (1) Check for relevant rows and columns: -----
    ## Logical index for NA values: 
    log_na <- is.na(table)  # get matrix of NAs

    ## (a) Rows: -----
      ## Get indices of rows that can be calculated:
      row_ix <- which(rowSums(log_na) == 1)  # get rows with exactly one NA (can be calculated). 
      ## Note: Apparently, numeric indexing is faster. 

      ## Sum up values in the indexed rows except their sum in the last column: 
      tab <- matrix(table[row_ix, -n_col], ncol = n_col - 1)
      cell_sums_row <- rowSums(tab, na.rm = TRUE)
      
      ## Test, which operation to perform: 
        ## Test for empty sum columns in the calculable rows:
        no_rsum_col <- is.na(table[row_ix, n_col])  # which of the rows is NA in the last column? 
        
        if (any(no_rsum_col)) {
          ## Check, which last columns in the identified rows are NA:
          cell_ix_row <- which(no_rsum_col)  
          ## get index for the row(s) in which the last column is empty. 
          table[row_ix, n_col][is.na(table[row_ix, n_col])] <- cell_sums_row[cell_ix_row]
        ...
        if (any(!no_rsum_col)) {  
          ...
          cell_ix_row <- row_ix[!no_rsum_col]
          row_NA <- matrix(table[cell_ix_row, ], ncol = n_col)  # get the rows with NAs not in last position. 
          no_cell_col <- which(is.na(row_NA), arr.ind = TRUE)  # index for the cells without entries. 
          
          insertion <- table[row_ix, n_col][!no_rsum_col] - cell_sums_row[!no_rsum_col]
          table[cell_ix_row, unique(no_cell_col[, 2])] <- insertion
    ...
    ## (b) Columns: -----
      ...
      col_ix <- which(colSums(log_na) == 1)  # get columns with exactly one NA (can be calculated). 
    
      ## Sum up values in the indexed columns except their sum in the last row: 
      tab <- matrix(table[-n_row, col_ix], nrow = n_row - 1)
      cell_sums_col <- colSums(tab, na.rm = TRUE)
      
      ## Test, which operation to perform: 
        ## Test for empty sum rows in the calculable columns:
        no_csum_row <- is.na(table[n_row, col_ix])  # which of the rows is NA in the last column? 
        
        if (any(no_csum_row)) {
          ## Check, which last rows in the identified columns are NA:
          cell_ix_col <- which(no_csum_row)  
          ## get index for the columns in which the last row is empty. 
          table[n_row, col_ix][is.na(table[n_row, col_ix])] <- cell_sums_col[cell_ix_col]
          ## Enter appropriate values. 
        } 
        ...
        if (any(!no_csum_row)) {  
          ...
          cell_ix_col <- col_ix[!no_csum_row]
          col_NA <- matrix(table[, cell_ix_col], nrow = n_row)  # get the columns with NAs not in last position. 
          no_cell_row <- which(is.na(col_NA), arr.ind = TRUE)  # index for the cells without entries. 
          
          insertion <- table[n_row, col_ix] - cell_sums_col[!no_csum_row]
          table[unique(no_cell_row[, 1]), cell_ix_col] <- insertion
          
        }
        
        
    ## XX: Return statement and stopping rules: --------
      log_na <- is.na(table)  # get matrix of NAs after one run. 
      ...
      ## STOPPING RULE 2:
      } else if(!any(rowSums(log_na) == 1 | colSums(log_na) == 1)) {  # if no single value in neither dimension. 
  ...
  ## (B) Include probabilities: ------
    ## (1) Calculate all possible probabilities (see above?): ------
      ...
      ## (b) Calculate all complements: ------
        
        ## For rows:
          col_pr <- ptab[1:frow, -(1:fcol)]  # get the probabilities in the last columns (row-probs). 
          row_na <- which(rowSums(is.na(col_pr)) == 1)  # get rows with NA-values. 
          rmat_na <- matrix(col_pr[row_na, ], ncol = ncol(col_pr))  # convert into matrix (to allow rowsums).
          ## Complete column matrix: 
          col_pr[row_na, is.na(col_pr[row_na, ])] <- 1 - rowSums(rmat_na, na.rm = TRUE)
          
          ## Check summing up to one:
            comp_row <- rowSums(col_pr) == 1
            
            ## Warn (or provide error, if they do not):
            if(any(!comp_row)) warning(paste0("Probabilities in row ", which(!comp_row), " do not sum up to 1."))
          
          ## Include into original table:
            ptab[1:frow, -(1:fcol)] <- col_pr
            
        ## For columns:
          row_pr <- ptab[-(1:frow), 1:fcol]  # get the probabilities in the last rows (col-probs). 
          col_na <- which(colSums(is.na(row_pr)) == 1)  # get cols with NA-values. 
          cmat_na <- matrix(row_pr[, col_na], nrow = nrow(row_pr))  # convert into matrix (to allow colsums).
          ## Complete row-matrix:
          row_pr[is.na(row_pr[, col_na]), col_na] <- 1 - colSums(cmat_na, na.rm = TRUE)
          
          ## Check summing up to one:
            comp_col <- colSums(row_pr) == 1
            
            ## Warn (or provide error, if they do not):
            if(any(!comp_col)) {
              warning(paste0("Probabilities in row ", which(!comp_col), " do not sum up to 1.\n"))
            }
          
          ## Include into original table:
            ptab[-(1:frow), 1:fcol] <- row_pr
      ...
      ## This accounts for using the freequency + probability algorithm and the frequency algorithm. 
