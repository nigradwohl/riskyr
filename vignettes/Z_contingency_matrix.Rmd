...
calc_tab_rec <- function(table) {
  ...
  ## (A) Frequency case: 
    ...
    ## Rows: 
      ## Sum up values except their sum in the last row: 
      cell_sums_col <- colSums(table[-n_row, ], na.rm = TRUE)
      
      ## Test, which operation to perform: 
      if (n_row %in% col_ix) {  # test, whether the maximal row is included (row sum).
        
        ## If TRUE, subtract the sum of the other values from the sum:
        
      } else {
        ## If FALSE (i.e., the sum is not included) simply add up: 
      }
